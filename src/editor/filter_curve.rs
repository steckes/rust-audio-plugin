...
use nih_plug_egui::egui::{self, Color32, Sense, Ui, Vec2b, Widget};
use num::Float;
...
pub struct FilterCurvePlot {
    plot_type: PlotType,
    filter_params: Lowpass,
    default_bounds: PlotBounds,
}

impl FilterCurvePlot {
    pub fn new(plot_type: PlotType, filter_params: Lowpass) -> Self {
        Self {
            plot_type,
            filter_params,
            default_bounds: PlotBounds::from_min_max(
                [scale_from_log10(X_BOUNDS.0, X_BOUNDS.0, X_BOUNDS.1), -10.0],
                [scale_from_log10(X_BOUNDS.1, X_BOUNDS.0, X_BOUNDS.1), 10.0],
            ),
        }
    }

    #[allow(unused)]
    pub fn with_y_bounds(mut self, y_min: f64, y_max: f64) -> Self {
        self.default_bounds = PlotBounds::from_min_max([0.0, y_min], [1.0, y_max]);
        self
    }
    ...
    pub fn show(self, ui: &mut Ui) {
        ...
        let spl_formatter =
            |mark: GridMark, _range: &RangeInclusive<f64>| format!("{} dB", mark.value);
                    ...
                    (0..NUM_PLOT_POINTS)
                        ...
                        .map(|x_pos| {
                            [
                                x_pos,
                                self.filter_params
                                    .response(scale_to_log10(x_pos, X_BOUNDS.0, X_BOUNDS.1) as f32)
                                    .norm()
                                    .log10() as f64
                                    * 20.0,
                            ]
